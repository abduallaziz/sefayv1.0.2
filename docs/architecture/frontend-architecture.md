...
- **CSS Grid column order under `dir="rtl"` (found 2026-07-08, Access Control split-view page):** in a `dir="rtl"` ancestor, the *first-defined* column in `grid-template-columns` renders on the **right**, not the left — the opposite of what you'd naively expect from "first item, reading order." Writing `grid-cols-[300px_1fr]` with the roles panel first in JSX put the roles panel on the right and the wide detail panel on the left, which was backwards from the intended layout (confirmed via a live screenshot before the fix). **Fix pattern**: wrap the grid container in `dir="ltr"` so column order is pinned to visual left→right regardless of locale, then set `dir="rtl"` again on each individual panel inside it so Arabic text still flows correctly — only the column *order* is forced, not the text direction. See `AccessControlPage.tsx` for the applied example. Any future side-by-side (non-stacking) grid layout in this codebase should check this before assuming DOM order maps to reading order.
