...
Sefay is a multi-tenant SaaS ERP. All tenants share a single Supabase (PostgreSQL) database. Tenant isolation is achieved through `company_id` scoping: every table that contains tenant-owned business data has a `company_id` column, and Supabase Row-Level Security (RLS) policies enforce that authenticated users can only access rows belonging to their own company.
...
1. **Every table containing tenant data has a `company_id` column** referencing the `companies` table. No exceptions. A table without `company_id` either contains global reference data (e.g. currency codes) or is a cross-tenant system table.

2. **RLS is enabled on every tenant data table.** The RLS policy for each table permits read and write only where `company_id` matches the `company_id` derived from the authenticated user's JWT.

3. **The `company_id` used in service queries is always derived from the authenticated session** (the JWT). It is never supplied by the client in a query parameter or request body.

4. **Application-layer `WHERE company_id = ?` filters are defense-in-depth**, not the primary isolation mechanism. The database enforces isolation regardless of application behavior.
...
6. **`company_id` values are stable UUIDs assigned at company creation** and are never reused, even after a factory reset. A factory reset wipes business data but does not change the company's `id`.
...
1. **Authentication record created** — Supabase Auth creates a user record for the Owner.
2. **Company record created** — a new row is inserted into the `companies` table, generating a new `company_id` UUID. Default settings are applied (currency, tax rate, language).
3. **Owner user record created** — a new row is inserted into the `users` table with the Owner role, linked to the new `company_id`.
4. **Default configuration** — any required default data (e.g. a default warehouse, system-level settings) is seeded for the new company.
...
These settings are stored on the `companies` table. Changes made through the Settings page are Owner-restricted (Admin can view, not modify — see [`permission-system.md`](./permission-system.md)).

---

## Future Tenant Settings (Phase 9)

Phase 9 (Company Branding and Information) significantly expands the tenant configuration model. The new fields, none of which currently exist in the `SettingsPage` or (as far as verified from the frontend) the backend `companies`/profile model, include:
...
- RLS policies that use the JWT-derived `company_id` as the isolation key.
- Tenant-scoped storage paths (`company_id/` prefix on all asset paths).
...
- `companies` row (the tenant's record)
- Owner's `users` row
...
**Isolation invariant during reset:** the factory reset is scoped strictly to the requesting tenant's `company_id`. It cannot affect any other tenant's data. The reset runs inside a single database transaction; any failure triggers a full rollback.
...
Every file asset stored on behalf of a tenant is namespaced by the tenant's `company_id` at the path level. Cross-tenant path collisions are structurally impossible because the `buildStoragePath()` helper always prefixes with `company_id`.

Signed URLs for asset access are generated server-side using the authenticated tenant's session. A user cannot request a signed URL for another tenant's asset path, because the `company_id` prefix is always derived from the authenticated session, not from a client parameter.
...
- A concept of a "parent" company or group that spans multiple `company_id` tenants.
