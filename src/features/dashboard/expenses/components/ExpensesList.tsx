...
// `notes` on an auto-generated expense is a raw string built server-side
// (expenses.service.ts: `Auto-generated from recurring template: {name}`) and
// stored as-is in the DB — it isn't app UI text, so next-intl can't translate
// it directly. Detected here and swapped for a short, translated label that
// keeps the template name; anything else in `notes` renders unchanged.
const AUTO_GENERATED_PREFIX = 'Auto-generated from recurring template: ';
// `fallbackName` is the expense's own category/title — already loaded on the
// row — shown instead of a bare dash when there is no note at all, so the
// column always carries some context instead of reading empty.
function formatExpenseNote(
  ...
  if (first.startsWith(AUTO_GENERATED_PREFIX)) {
    return t('table.autoGenerated', { name: first.slice(AUTO_GENERATED_PREFIX.length) });
  }
  return first;
}
                    ...
                    return (
                      <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-gray-800/30 transition-colors">
                        ...
                        <td className="px-3 py-3 text-slate-400 text-xs max-w-[160px] truncate">
                          {formatExpenseNote(row.notes, row.category?.name ?? row.title, t)}
                        </td>
