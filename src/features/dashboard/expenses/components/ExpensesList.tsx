              ...
              {expenses.map((row) => {
                ...
                return (
                  <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-800 dark:text-white font-semibold">{formatCurrency(row.amount, currency)}</td>
                    <td className="hidden sm:table-cell px-4 py-3 text-slate-500">{row.requester?.name ?? '—'}</td>
                    <td className="hidden sm:table-cell px-4 py-3 text-slate-400 text-xs max-w-[120px] truncate">{row.notes?.split('|')[0]?.trim() || '—'}</td>
                      ...
                      {row.status === 'pending' && (
                        <div className="flex items-center gap-2 justify-end">
                          <button
                            ...
                            className="px-3 py-1 rounded-lg text-xs font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/20 border border-emerald-500/20"
                          >
                            {t('actions.approve')}
                          </button>
                          <button
                            onClick={() => setRejectTarget(row)}
                            className="px-3 py-1 rounded-lg text-xs font-medium bg-red-500/10 text-red-600 dark:text-red-400 hover:bg-red-500/20 border border-red-500/20"
                          >
                            {t('actions.reject')}
                          </button>
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl w-full max-w-md p-6 space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => { setRejectTarget(null); setRejectReason('') }} className="flex-1 py-2 border border-slate-200 dark:border-gray-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-gray-800 rounded-lg text-sm">
                إلغاء
              </button>
