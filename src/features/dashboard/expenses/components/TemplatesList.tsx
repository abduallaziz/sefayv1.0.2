...
import { RefreshCw, ShieldCheck, Pencil, Trash2, Check, X } from 'lucide-react'
import { useExpenseTemplates, useUpdateExpenseTemplate, useDeleteExpenseTemplate } from '../hooks/useExpenses'
  ...
  return (
    <>
      <tr className="hover:bg-slate-50 dark:hover:bg-gray-800/30 transition-colors">
        ...
        <td className="px-4 py-3 text-center">
          <button
            onClick={togglePreApproved}
            disabled={isPending || template.recurrence_type === 'none'}
            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium border transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
        ...
        <td className="px-4 py-3">
          <div className="flex items-center gap-1 justify-end">
            {editMode ? (
              <>
                <button onClick={saveEdit} disabled={isPending} className="p-1.5 rounded-lg hover:bg-emerald-500/10 text-slate-400 hover:text-emerald-500">
                  <Check className="w-4 h-4" />
                </button>
                <button onClick={() => { setEditMode(false); setEditName(template.name); setEditAmount(String(template.default_amount ?? '')) }} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-gray-700 text-slate-400">
                  <X className="w-4 h-4" />
                </button>
              </>
            ) : (
              <>
                <button onClick={() => setEditMode(true)} disabled={isPending} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-gray-700 text-slate-400 hover:text-slate-600 dark:hover:text-white">
                  <Pencil className="w-4 h-4" />
                </button>
                <button onClick={() => setConfirmDelete(true)} disabled={isPending} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 text-slate-400 hover:text-red-500">
          ...
          <td colSpan={7} className="px-4 py-3 bg-red-50 dark:bg-red-500/10 border-t border-red-200 dark:border-red-500/20">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => setConfirmDelete(false)} className="px-3 py-1 text-xs border border-slate-200 dark:border-gray-700 text-slate-600 dark:text-slate-400 rounded-lg hover:bg-slate-100">
                  إلغاء
                </button>
                <button onClick={handleDelete} disabled={deleteMutation.isPending} className="px-3 py-1 text-xs bg-red-500 hover:bg-red-600 text-white rounded-lg disabled:opacity-50">
