  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="flex items-center justify-between px-6 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Control what each role can see and do across the workspace.
            </p>
          </div>
          <Button
          ...
          >
            <Plus className="me-2 h-4 w-4" /> Create Custom Role
          </Button>
        </div>
      </div>
        ...
        <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
          {isError ? (
            ...
            <EmptyState
              ...
              description="System roles (Owner, Manager, Cashier) are always available. Create a custom role to fine-tune access for a specific job."
              action={
                <Button
                  onClick={() => setSheet({ mode: 'create' })}
      ...
      <ConfirmDialog
        ...
        message={
          roleToDelete
            ? `"${roleToDelete.name}" will be permanently deleted. This cannot be undone.`
            : ''
        }
