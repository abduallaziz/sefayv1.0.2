...
export function TenantsTable({ tenants, onActivate, onDeactivate, onExtendTrial }: TenantsTableProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-lg border border-[#1e2130] bg-[#1a1f2e]"
    >
      {/* Header */}
      <div className="flex items-center justify-between border-b border-[#1e2130] p-4">
        <h3 className="text-base font-semibold text-white">{t('title')}</h3>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 rounded-lg border border-[#1e2130] bg-[#141720] px-3 py-2 w-56">
            <Search className="h-4 w-4 text-[#64748b]" />
            <input
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder={t('search')}
              className="bg-transparent text-sm text-white placeholder:text-[#64748b] focus:outline-none w-full"
            />
          </div>
            ...
            {(['all', 'active', 'trial', 'suspended', 'cancelled'] as const).map((s) => (
              <button
                ...
                className={`rounded px-3 py-1.5 text-xs font-medium transition-colors capitalize ${
                  statusFilter === s ? 'bg-[#6366f1]/20 text-[#818cf8]' : 'text-[#64748b] hover:text-white'
                }`}
              >
            ...
            {filtered.map((tenant, i) => {
              ...
              return (
                <motion.tr
                  ...
                  className="hover:bg-[#242938] transition-colors"
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#6366f1]/20 text-xs font-bold text-[#818cf8]">
                        {tenant.name.charAt(0).toUpperCase()}
                      </div>
                      <span className="text-sm font-medium text-white">{tenant.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-sm text-[#64748b] capitalize">{tenant.business_type}</td>
                  <td className="px-4 py-3">
                    <Badge variant={status.variant} className="flex w-fit items-center gap-1">
                      {status.icon}{status.label}
                    </Badge>
                  </td>
                  <td className="px-4 py-3 text-sm text-[#64748b]">{tenant.users_count ?? '-'}</td>
                  <td className="px-4 py-3 text-sm text-[#64748b]">{tenant.branches_count ?? '-'}</td>
                  <td className="px-4 py-3 text-sm text-white font-medium">
                    {tenant.mrr ? `$${tenant.mrr.toLocaleString()}` : '-'}
                  </td>
                  <td className="px-4 py-3 text-sm text-[#64748b]">{tenant.created_at}</td>
                  <td className="px-4 py-3">
                    <DropdownMenu>
