    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="border-[#1e2130] bg-[#141720] text-white sm:max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
          <div>
            <label className="mb-1.5 block text-sm text-slate-400">{t('payment.tenant')}</label>
            <select {...register('tenant_id')} className="w-full rounded-lg border border-[#1e2130] bg-[#0f1117] px-3 py-2 text-sm text-white focus:outline-none">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1.5 block text-sm text-slate-400">{t('payment.plan')}</label>
              <select {...register('plan_id')} className="w-full rounded-lg border border-[#1e2130] bg-[#0f1117] px-3 py-2 text-sm text-white focus:outline-none">
          ...
          <div>
            <label className="mb-1.5 block text-sm text-slate-400">{t('payment.amount')}</label>
            <Input
              {...register('amount', { valueAsNumber: true })}
              type="number"
              className="border-[#1e2130] bg-[#0f1117] text-white"
            />
          </div>
          <div>
            <label className="mb-1.5 block text-sm text-slate-400">{t('payment.note')}</label>
            <Input {...register('note')} placeholder={t('payment.notePlaceholder')} className="border-[#1e2130] bg-[#0f1117] text-white" />
          </div>
