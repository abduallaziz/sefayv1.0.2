...
function StatCard({ label, value, sub, icon: Icon, trend, trendUp }: {
  ...
  return (
    <div className="bg-white dark:bg-[#141720] border border-slate-200 dark:border-[#1e2130] rounded-xl p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <span className="text-xs text-slate-500 dark:text-slate-500 uppercase tracking-wider">{label}</span>
        <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-[#1e2130] flex items-center justify-center">
          <Icon className="w-4 h-4 text-slate-500 dark:text-slate-400" />
        </div>
      </div>
      <div>
        <div className="text-2xl font-bold text-slate-800 dark:text-white">{value}</div>
        <div className="text-xs text-slate-500 dark:text-slate-500 mt-1">{sub}</div>
      </div>
      {trend && (
...
export default function ReportsAuditPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0f1117] text-slate-800 dark:text-white p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              activeTab === tab.key ? 'bg-blue-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white'
            }`}
          >
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="bg-white dark:bg-[#141720] border border-slate-200 dark:border-[#1e2130] rounded-xl p-5">
              <h3 className="text-sm font-semibold text-slate-800 dark:text-white mb-4">{t('tenantsByPlan')}</h3>
              {revenueByPlanQuery.isLoading ? (
                <div className="h-[160px] flex items-center justify-center text-slate-500 dark:text-slate-500 text-sm">Loading...</div>
              ) : (
                <>
                    ...
                    {byPlan.map((p, i) => (
                      <div key={p.planName} className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-500 dark:text-slate-400">{p.planName}</span>
                        </div>
                        <span className="text-slate-800 dark:text-white font-medium">{p.tenantCount}</span>
                      </div>
                    ))}
                  </div>
                </>
          ...
          <div className="bg-white dark:bg-[#141720] border border-slate-200 dark:border-[#1e2130] rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-slate-800 dark:text-white">{t('churnTrend')}</h3>
      ...
      {activeTab === 'revenue' && (
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-[#141720] border border-slate-200 dark:border-[#1e2130] rounded-xl p-5">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-white mb-4">{t('mrrVsArr')}</h3>
            {mrrHistoryQuery.isLoading ? (
              <div className="h-[260px] flex items-center justify-center text-slate-500 dark:text-slate-500 text-sm">Loading...</div>
            ) : (
          ...
          <div className="bg-white dark:bg-[#141720] border border-slate-200 dark:border-[#1e2130] rounded-xl p-5">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-white mb-4">{t('revenueByPlan')}</h3>
            <div className="space-y-2">
              {byPlan.map((plan, i) => (
                <div key={plan.planName} className="flex items-center justify-between py-2.5 border-b border-slate-200 dark:border-[#1e2130] last:border-0">
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full" style={{ background: PLAN_COLORS[i % PLAN_COLORS.length] }} />
                    <span className="text-sm text-slate-800 dark:text-white">{plan.planName}</span>
                  </div>
                  <div className="flex items-center gap-6 text-xs">
                    <span className="text-slate-500 dark:text-slate-400">{plan.tenantCount} tenants</span>
                    <span className="text-slate-500 dark:text-slate-400">{plan.percentage}%</span>
                    <span className="text-slate-800 dark:text-white font-bold w-20 text-right">${plan.mrr.toLocaleString()}/mo</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      {activeTab === 'tenants' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white dark:bg-[#141720] border border-slate-200 dark:border-[#1e2130] rounded-xl p-5">
              <h3 className="text-sm font-semibold text-slate-800 dark:text-white mb-4">{t('newPerMonth')}</h3>
            ...
            <div className="bg-white dark:bg-[#141720] border border-slate-200 dark:border-[#1e2130] rounded-xl p-5">
              <h3 className="text-sm font-semibold text-slate-800 dark:text-white mb-4">{t('planDist')}</h3>
              {revenueByPlanQuery.isLoading ? (
                <div className="h-[200px] flex items-center justify-center text-slate-500 dark:text-slate-500 text-sm">Loading...</div>
              ) : (
                <ResponsiveContainer width="100%" height={200}>
