...
export function PurchaseOrderLineItems({ rows, items, onChange }: Props) {
  const t = useTranslations('purchasing');

  const addRow = () => onChange([...rows, { item_id: '', variant_id: '', quantity_ordered: 1, unit_cost: 0 }]);
  const removeRow = (i: number) => onChange(rows.filter((_, idx) => idx !== i));
  const updateRow = (i: number, field: keyof POLineRow, value: string | number) =>
    onChange(rows.map((r, idx) => (idx === i ? { ...r, [field]: value } : r)));

  const total = rows.reduce((sum, r) => sum + r.quantity_ordered * r.unit_cost, 0);
      ...
      {rows.map((row, i) => (
        <div key={i} className="grid grid-cols-[2fr_1fr_1fr_auto] gap-2 items-center">
          ...
          <input
            ...
            onChange={(e) => updateRow(i, 'quantity_ordered', Number(e.target.value))}
            className={inputClass}
          />
          <input
      ...
      {rows.length > 0 && (
        <div className="flex justify-end pt-2 border-t border-slate-100 dark:border-gray-800">
          <p className="text-sm font-semibold text-slate-800 dark:text-white">
            {t('lineTotal')}: {total.toLocaleString('en-US')}
          </p>
        </div>
      )}
    </div>
