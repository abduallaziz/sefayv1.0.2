...
- Inconsistent empty-state treatment: some tables show icon + title + description, others show plain text only — needs a shared `EmptyState` component used everywhere (component already exists at `src/shared/ui/empty-state.tsx` but is unused by Inventory; next planned milestone).
- Delete/Cancel confirmation dialogs (5: Locations, Warehouses, Purchase Orders, Goods Receipts, Transfers) and form modals (7) are all custom, near-identical implementations with no shared component — should be consolidated into a shared, visually-polished confirmation dialog and form-modal wrapper (existing-but-unused `src/shared/ui/modal.tsx` / `dialog.tsx`).
